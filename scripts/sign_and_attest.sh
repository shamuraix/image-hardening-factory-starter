...
: "${COSIGN_KEY_PATH:?COSIGN_KEY_PATH must name the protected GitLab file variable containing the encrypted Cosign private key}"
...
if [[ ${FACTORY_RELEASE_ENV:-commercial} =~ ^gov[12]$ ]]; then
  [[ ${GITLAB_USER_EMAIL:-} == *.us ]] || { echo "Gov signing requires a .us approver" >&2; exit 1; }
fi
...
# protected, environment-scoped GitLab file variable and is never uploaded.
...
declare -A predicates=(
  ...
  ["${evidence}/findings.json"]="https://factory.platform.example/vulnerability/v1"
  ...
  ["${evidence}/scans/fcs/status.json"]="https://factory.platform.example/fcs-decision/v1"
  ["${evidence}/compliance/result.json"]="https://factory.platform.example/compliance/v1"
  ["${evidence}/tests/result.json"]="https://factory.platform.example/test/v1"
  ["${evidence}/gate-result.json"]="https://factory.platform.example/gate/v1"
)
